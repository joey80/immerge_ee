...
class MembersAuth
{
    ...
    public function _redirect_next_site($sites, $current_idx, $current_url)
    {
        ...
        if ($orig_id === FALSE)
        {
            ...
            $return = ee()->functions->remove_double_slashes(ee()->functions->form_backtrack());
    ...
    public function forgot_password($ret = '-3')
    {
        $data = array(
            ...
            'hidden_fields'     => array(
                'ACT'   => ee()->functions->fetch_action_id('Member', 'retrieve_password'),
                'RET'   => $ret,
    ...
    public function reset_password()
    {
        ...
        // Fetch the user data
        $query = ee()->db->select("username, email")
        ...
        $rand = ee()->functions->random('alnum', $len);
        ...
        ee()->db->set('password', ee()->functions->hash($rand))
                     ->where('member_id', $member_id)
                     ->update('members');
        ...
        $template = ee()->functions->fetch_email_template('reset_password_notification');
        $email_tit = $this->_var_swap($template['title'], $swap);
        $email_msg = $this->_var_swap($template['data'], $swap);
...
/* End of file MembersAuth.php */
/* Location: ./system/user/addons/visitor/Service/MembersAuth.php */